...
1. **Create `config.yml`** (non-secret settings):
   - Using the [config template](config.yml.template) as source, create a new `config.yml` file:
     ```bash
     cp config.yml.template config.yml
     ```
   - Edit it to suit your needs (mediawiki sites and namespaces, models, search parameters, etc.)

2. **Create `.env`** (secrets only):
   - Using the [env template](dotenv.template) as source, create a new `.env` file:
     ```bash
...
4. **Configure the Application**:
   Ensure both `config.yml` and `.env` have been set up by following the [Configuration](#configuration) instructions above.
...
2. Run the container:
   - **Note 1:** Ensure both `config.yml` and `.env` have been set up by following the [Configuration](#configuration) instructions above. `config.yml` (non-secret settings) is required and must be mounted. Secrets can be supplied either via a `.env` file (mounted as shown) or as plain `--env` flags — use whichever suits your deployment.
